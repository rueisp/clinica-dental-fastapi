...
export default function PlanesPage() {
  ...
  const handleCambiarPlan = async (plan) => {
    const confirmar = confirm(`¿Cambiar al plan ${plan.nombre}?`);
    if (!confirmar) return;
      ...
      {planActual && planActual.tiene_plan && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <div className="border-l-4 border-purple-500 pl-4 mb-4">
            <p className="text-xs font-bold text-purple-600 uppercase tracking-wider">Tu plan actual</p>
          ...
          <div className="mt-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-purple-600 font-bold">{planActual.porcentaje_progreso}%</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div className="bg-purple-500 rounded-full h-2" style={{ width: `${planActual.porcentaje_progreso}%` }} />
            </div>
          </div>
        </div>
      )}
        ...
        {planes.map((plan) => {
          ...
          return (
            <div key={plan.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {esPlanActual ? (
                <button disabled className="w-full bg-purple-100 text-purple-600 py-2 rounded-lg font-bold cursor-not-allowed">
                  ✅ Plan actual
                </button>
              ) : (
                <button
